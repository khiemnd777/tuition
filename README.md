...
- Platform/Admin auth-session decoupling cho phép `platform_admin` giữ session hợp lệ ngay cả khi session đó không bind vào tenant nào; các workflow control-plane sẽ tự dùng scope `all`, còn workflow tenant-owned vẫn yêu cầu active tenant như cũ.
...
- `platform_admin`: quản trị tenant, subscription, entitlement, finance console và vận hành cross-tenant.
- `tenant_owner`: sở hữu tenant, quản lý subscription tenant, operator và toàn bộ workflow trong tenant đó.
- `tenant_admin`: quản lý operator và workflow vận hành hằng ngày trong tenant.
- `tenant_staff`: quản lý master data, lớp và thông báo trong tenant.
- `tenant_accountant`: quản lý bảng phí, hóa đơn, thanh toán, đối soát, báo cáo, email/cron và audit trong tenant.
...
- `POST /api/v1/auth/tenant-signup`: public self-serve signup để tạo tenant, school đầu tiên, owner đầu tiên, rồi auto-login bằng cookie phiên.
...
- `GET /api/v1/subscriptions/plans`: danh sách subscription plan active cho tenant admin, gồm `basePriceVnd`, `promotionalPriceVnd`, `displayPriceVnd`, `contactPrice`, và `displayOrder`; không trả `partnerPriceVnd`.
- `GET /api/v1/platform/subscription-plans`: Platform Admin đọc plan catalog gồm cả plan active và archived, kèm `basePriceVnd`, `partnerPriceVnd`, `promotionalPriceVnd`, `displayPriceVnd`, `contactPrice`, và `displayOrder`.
- `POST /api/v1/platform/subscription-plans`: Platform Admin tạo/cập nhật/archive subscription plan bằng `code`, `name`, `status`, `description`, `contactPrice`, `displayOrder`, `basePriceVnd`, `partnerPriceVnd`, `promotionalPriceVnd`, và `limits` JSON object; khi gửi kèm `id`, endpoint update đúng plan đó và cho phép đổi `code` miễn là không trùng plan khác.
- `GET /api/v1/subscriptions/purchase`: dữ liệu self-serve checkout cho tenant owner, gồm plan hiện tại, plan options, provider options, và kỳ billing đề xuất.
- `POST /api/v1/subscriptions/purchase/checkout`: tạo hoặc reuse subscription invoice cho checkout self-serve rồi trả QR/link thanh toán theo provider tenant hiện có.
- `POST /api/v1/tenants/subscription/save`: Platform Admin cập nhật plan/status/date của subscription theo `tenantId`; plan archived không thể được assign.
...
- `POST /api/v1/subscriptions/billing/config`: lưu billing config của tenant active, gồm `amount`, `intervalMonths`, `dueDays`, `autoRenew`, `renewalMode`, và `financeNote`.
- `GET /api/v1/subscriptions/billing/export?dataset=invoices|overdue|paid|dunning`: export CSV billing lane của subscription theo filter `status`, `periodFrom`, `periodTo`, `dueFrom`, `dueTo`.
- `GET /api/v1/subscriptions/finance-console`: cross-tenant finance console summary và row list; hỗ trợ `scope=active|all`, `q`, `subscriptionStatus`, `invoiceStatus`, `autoRenew`, `renewalMode`, `overdueOnly`, `missingConfigOnly`.
...
- `POST /api/v1/admin/users/roles`: gán role tenant `tenant_owner`, `tenant_admin`, `tenant_staff`, `tenant_accountant` cho user trong active tenant; yêu cầu permission `user.assign_role`.
- `GET /api/v1/admin/roles`: danh sách role và permission.
- `GET /api/v1/platform/users`: danh sách platform users và system roles cho `platform_admin`.
- `POST /api/v1/platform/users/save`: tạo/cập nhật platform user nội bộ mà không tự gắn tenant membership.
- `POST /api/v1/platform/users/roles`: gán system role `platform_admin` cho platform user.
- `GET /api/v1/platform/roles`: danh sách system role dành cho platform surface.
